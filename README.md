...
- 🎯 **62 detected patterns** — repeated `f`, hammering `j`, `dw`→`i` instead of `cw`, insert-mode habits like `<BS>` spam, bouncing in and out with `<Esc>i`, a one-shot `<Esc>`→motion→`i` round trip, retyping the same identifier instead of using `<C-n>`, getting stuck in terminal mode, manually scrolling back to a place you jumped or edited before, manually repeating the same edit sequence 3 times instead of recording a macro, retyping the same `:s///` substitution across lines instead of using `&`/`g&`, bouncing between the same two files with `:e`/`:b` instead of `<C-^>`, using `:tabnew` as a one-tab-per-file browser, repeating `ci"`/`ci'` instead of yanking around the quotes with `ya"`/`ya'`, and more across motion, edit, search, window, fold, mark, macro, diff, ex, and terminal

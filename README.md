...
- 🎯 **44 detected patterns** — repeated `f`, hammering `j`, `dw`→`i` instead of `cw`, insert-mode habits like `<BS>` spam, bouncing in and out with `<Esc>i`, or retyping the same identifier instead of using `<C-n>`, and more across motion, edit, search, window, fold, mark, and macro

...
- 🎯 **49 detected patterns** — repeated `f`, hammering `j`, `dw`→`i` instead of `cw`, insert-mode habits like `<BS>` spam, bouncing in and out with `<Esc>i`, a one-shot `<Esc>`→motion→`i` round trip, retyping the same identifier instead of using `<C-n>`, getting stuck in terminal mode, and more across motion, edit, search, window, fold, mark, macro, diff, and terminal
...
49 patterns total — see `:help tobira-patterns` for the full list.

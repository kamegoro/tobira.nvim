...
-- A separate pure state machine from the normal-mode seq/feed above: logger.lua
-- only calls feed_insert() while the mode cache says the user is in insert
-- mode, feeding it a canonical key name ('<BS>', '<Left>', '<Right>', '<Esc>')
-- or nil for an ordinary typed character.

local function iseq()
  return patterns.new_insert_seq()
end
...
describe('when the user backspaces 5 times in a row in insert mode', function()
  it('fires insert_bs_repeat suggesting <C-w>', function()
    local s = iseq()
    patterns.feed_insert(s, '<BS>')
    patterns.feed_insert(s, '<BS>')
    patterns.feed_insert(s, '<BS>')
    patterns.feed_insert(s, '<BS>')
    local result = patterns.feed_insert(s, '<BS>')
    assert.is_not_nil(result)
  ...
  it('resets the streak once an ordinary key is typed in between', function()
    ...
    patterns.feed_insert(s, '<BS>')
    patterns.feed_insert(s, '<BS>')
    patterns.feed_insert(s, nil) -- ordinary character, breaks the streak
    patterns.feed_insert(s, '<BS>')
    local result = patterns.feed_insert(s, '<BS>')
...
describe('when the user presses <Left> 5 times in a row in insert mode', function()
  it('fires insert_left_repeat suggesting b', function()
    local s = iseq()
    patterns.feed_insert(s, '<Left>')
    patterns.feed_insert(s, '<Left>')
    patterns.feed_insert(s, '<Left>')
    patterns.feed_insert(s, '<Left>')
    local result = patterns.feed_insert(s, '<Left>')
    assert.is_not_nil(result)
...
describe('when the user presses <Right> 5 times in a row in insert mode', function()
  it('fires insert_right_repeat suggesting w', function()
    local s = iseq()
    patterns.feed_insert(s, '<Right>')
    patterns.feed_insert(s, '<Right>')
    patterns.feed_insert(s, '<Right>')
    patterns.feed_insert(s, '<Right>')
    local result = patterns.feed_insert(s, '<Right>')
    assert.is_not_nil(result)
...
describe('when the user enters and immediately leaves insert mode with no input, twice in a row', function()
  ...
  it('does not fire when the user typed something before leaving', function()
    local s = iseq()
    patterns.feed_insert(s, '<Esc>') -- first empty bounce
    patterns.feed_insert(s, 'x') -- ordinary typing this time
    local result = patterns.feed_insert(s, '<Esc>')
  ...
  it('resets the bounce streak after a non-empty escape', function()
    local s = iseq()
    patterns.feed_insert(s, '<Esc>') -- 1st empty bounce
    patterns.feed_insert(s, 'x')
    patterns.feed_insert(s, '<Esc>') -- non-empty, resets streak
    local result = patterns.feed_insert(s, '<Esc>') -- only 1st empty bounce again
...
describe('insert-mode streaks vs. the bounce counter', function()
  it('pressing <BS>/<Left>/<Right> counts as input, so it does not count as an empty bounce', function()
    local s = iseq()
    patterns.feed_insert(s, '<Esc>') -- 1st empty bounce
    patterns.feed_insert(s, '<BS>') -- counts as real input this session
    local result = patterns.feed_insert(s, '<Esc>')

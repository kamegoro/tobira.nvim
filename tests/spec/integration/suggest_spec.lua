...
describe('when the per-session auto suggestion limit is reached', function()
  ...
  it('stops showing after max_per_session auto suggestions', function()
    ...
    local shown3 = with_float_spy(function()
      suggest.show('.')
    end)
...
describe('adoption detection — nasty / adversarial cases', function()
  ...
  it('adopting one command does not prevent adoption of another', function()
    local graph = require('tobira.core.graph')
    -- two back-to-back shows would be blocked by min_interval_ms; disable it
    config.setup({ min_interval_ms = 0, max_per_session = 10 })
    with_float_spy(function()
      suggest.show(';')
    end)
    with_float_spy(function()

...
-- Every save-triggering function (mark_shown, mark_adopted,
...
-- patterns_insert.feed_insert() cares about. Built once via the same
...
-- '<C-n>' is the same shape of problem: in normal mode it's Vim's
...
--
-- '<C-o>' is included for exactly the same reason and resolves the
...
-- Verify-before-credit fix: cheap gate deciding whether a completed Ex
-- command is even worth deferring a changedtick-based success check for at
-- all (see the full fix comment at the call site below). Deliberately
...
-- duplication precedent as the ex_file_pingpong fix's PINGPONG_WORDS table
-- below: that module stays a pure, vim.*-free tokenizer, and
...
-- Ex-command tracking: vim.on_key sees every cmdline keystroke, but
...
--
-- Resets seq/insert_seq on every cmdline keystroke, same as the earlier
...
local function handle_cmdline_key(key)
  ...
  if key == CMDLINE_CR then
    ...
    -- class and same timing fix as ex_file_pingpong's own verify-before-
    -- credit below): this on_key callback for <CR> runs BEFORE Neovim validates or
    ...
    -- identical in shape to the one ex_file_pingpong's fix already accepts
    -- (see that fix's comment) — if something else mutates this same buffer in the
    ...
    -- commands, same as ex_file_pingpong's test suite).
    --
    ...
    -- :qa, ...) — mirroring the PINGPONG_WORDS gate below for the same reason.
    ...
    -- Independent of the usage-count tracking above -- both
    ...
    -- Only read nvim_tabpage_list_wins for an actual :tabnew
...
local function handle_key(key)
  ...
  -- patterns_insert.lua's `co_result`: both are fed the same
  ...
  --
  -- macro_result is deliberately checked FIRST, ahead of both: it only
  ...
  -- patterns_insert.lua's insert_completion_repeat on that same final

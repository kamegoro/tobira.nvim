...
## ローカル作業コンテキストを必ず参照（最優先）

次のローカルファイル（いずれも `.gitignore` されている）が存在したら、作業開始前に必ず読むこと。付属していなくても `read_file` で取得する。

| ファイル | 役割 |
|---|---|
| `.agent/plan.md` | **進行中作業の唯一の真実の源**。未コミットの思考・半完成の TODO・試行錯誤・ブロッカー。**Claude/Copilot 両方がここに書き、ここを読む**。 |
| `CLAUDE.md` | 詳細な設計原則と TDD 規約。このファイル(`copilot-instructions.md`)はその押さえ要点版。 |
| `PLAN.md` | 公開化されていない詳細プランとイシュー一覧。 |

これらファイルの内容はこのファイルの規約と矛盾しない限り優先する。矛盾した場合はその都度ユーザーに確認する。

作業完了時には `.agent/plan.md` を更新して次のセッションに渡すこと。
